...
public class BuyerDealApiController {
     ...
     * GET /api/buyer/deals
     *
     * JWT 토큰에서 사용자 정보를 추출하여 해당 구매자의 거래만 반환
     */
    @GetMapping
    public ResponseEntity<Map<String, Object>> getBuyerDeals(
        ...
        try {
            ...
            log.info("구매자 거래 조회 요청 - buyerId: {}", buyerId);

            // ⭐ 핵심: 현재 로그인한 사용자가 구매자인 거래만 조회
            List<Deal> myDeals = dealRepository.findByBuyer_UserId(buyerId);
    ...
    public ResponseEntity<Map<String, Object>> getDealDetail(
        ...
        try {
            ...
            UUID buyerId = userDetails.getUserId();
            log.info("거래 상세 조회 - dealId: {}, buyerId: {}", dealId, buyerId);

            // 거래 조회
            Deal deal = dealRepository.findById(dealId)
                    .orElseThrow(() -> new RuntimeException("거래를 찾을 수 없습니다."));

            // 권한 확인: 내가 구매자인 거래인지 검증
            if (!deal.getBuyer().getUserId().equals(buyerId)) {
                log.warn("권한 없는 거래 접근 시도 - dealId: {}, userId: {}", dealId, buyerId);
